...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(10)
		DrawMarker(21,2350.91,4744.78,34.36-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,255,230,100,120,0,0,0,1)
		if GetDistanceBetweenCoords(2350.91,4744.78,34.36, GetEntityCoords(LocalPed())) < 1.0 then
			basiccheck()
		end
		if onjob == true then 						
			DrawMarker(21,2352.49,4743.03,34.41-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,255,230,100,120,0,0,0,1)
			if GetDistanceBetweenCoords(destination[l].x,destination[l].y,destination[l].z, GetEntityCoords(GetPlayerPed(-1))) < 30.0 then
				DrawMarker(21,destination[l].x, destination[l].y, destination[l].z,0,0,0,0.0,0,0,0.5,0.5,0.4,255,230,100,100,0,0,0,1)		
			end
			if GetDistanceBetweenCoords(destination[l].x,destination[l].y,destination[l].z, GetEntityCoords(GetPlayerPed(-1))) < 1.0 then
				if not IsInVehicle() and CanLeaveItem then
					drawTxt("PRESSIONE ~g~E~s~ PARA ENTREGAR O PACOTE DE ~o~LARANJAS", 4,0.5,0.90,0.50,255,255,255,200)
...
Citizen.CreateThread(function()
	...
	Citizen.Wait(10)
		...
		if onjob then			
			if GetDistanceBetweenCoords(myCoords.x, myCoords.y, myCoords.z, 2352.49,4743.03,34.41, true) < 0.5 and onjob then
				nearBoxFarm = true
				openMenuBox(nearBoxFarm)
			elseif GetDistanceBetweenCoords(myCoords.x, myCoords.y, myCoords.z, 2352.49,4743.03,34.41, true) >= 0.5 and onjob then
				nearBoxFarm = false
				openMenuBox(nearBoxFarm)
			end
		end		
...
function basiccheck()
	if onjob == false then 
		drawTxt("PRESSIONE ~g~E~s~ PARA ~y~INICIAR~s~ AS ENTREGAS", 4,0.5,0.90,0.50,255,255,255,200)
		if IsControlJustPressed (1, 38) then
			TriggerServerEvent('inicio:sucesso')
			TriggerEvent("Notify","sucesso","Entregas iniciadas.")
		end
	else
		drawTxt("PRESSIONE ~g~H~s~ PARA ~r~ENCERRAR~s~ AS ENTREGAS", 4,0.5,0.90,0.50,255,255,255,200)
...
function openMenuDeliveryCar(nearDeliveryCar)
	if nearDeliveryCar and CanLeaveItem then
		if stockFrutas <= limitStock then
			drawTxt("PRESSIONE ~g~E~s~ PARA GUARDAR A ENCOMENDA NA CAMINHONETE", 4,0.5,0.90,0.50,255,255,255,200)
			if IsControlJustPressed(1, 38) then
				...
				TriggerEvent("Notify","sucesso","Possui <b>"..stockFrutas.."</b> caixa de frutas na caminhonete.")
			end
		else
			drawTxt("A caminhonete está cheia efetue mais entregas", 4,0.5,0.90,0.50,255,255,255,200)
		end
	end
end

function getBoxOnCar(nearDeliveryCar2)
	if nearDeliveryCar2 and not CanLeaveItem and GetDistanceBetweenCoords(destination[l].x,destination[l].y,destination[l].z, GetEntityCoords(GetPlayerPed(-1))) < 20.0 then
		if stockFrutas >= 2 then
			drawTxt("PRESSIONE ~g~E~s~ PARA PEGAR A CAIXA NA CAMINHONETE", 4,0.5,0.90,0.50,255,255,255,200)
			if (IsControlJustPressed(1, 38)) then
				Citizen.Wait(100)	
				CanLeaveItem = true
			end				
			...
			drawTxt("A caminhonete está vazia busque mais caixas de laranja.", 4,0.5,0.90,0.50,255,100,100,250)
		end
	end
end
...
function openMenuBox(nearBoxFarm)
	if nearBoxFarm and onjob then
		if stockFrutas <= limitStock then
			drawTxt("PRESSIONE ~g~E~s~ PARA PEGAR ~o~EMPACOTAR AS LARANJAS.~w~", 4,0.5,0.90,0.50,255,255,255,200)
			...
			drawTxt("A CAMINHONETE ESTÁ CHEIA, EFETUE ALGUMAS ENTREGAS", 4,0.5,0.90,0.50,255,255,255,200)
		end
	end
end
