...
local servico = false
local selecionado = 0
local CoordenadaX = -204.79
local CoordenadaY = -1342.61
local CoordenadaZ = 30.89
...
Citizen.CreateThread(function()
	while true do
		...
		if servico then
			...
			if distance <= 30 then
				DrawMarker(21,locs[selecionado].x,locs[selecionado].y,locs[selecionado].z+0.20,0,0,0,0,180.0,130.0,2.0,2.0,1.0,255,230,100,100,0,0,0,1)
				if distance <= 2.5 then
					drawTxt("PRESSIONE  ~r~E~w~  PARA ENTREGAR ~y~AS FERRAMENTAS",4,0.5,0.90,0.50,255,255,255,200)
					if IsControlJustPressed(0,38) and emP.checkPermission() then
						if IsVehicleModel(GetVehiclePedIsUsing(ped),GetHashKey("flatbed")) or IsVehicleModel(GetVehiclePedIsUsing(ped),GetHashKey("towtruck")) or IsVehicleModel(GetVehiclePedIsUsing(ped),GetHashKey("rebel2")) then
