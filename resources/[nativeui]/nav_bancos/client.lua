	...
	for _,v in pairs(bancos) do
		...
		if distance <= 3 then
			...
			if parseInt(hora) >= 07 and parseInt(hora) <= 17 then
				...
				TriggerEvent("Notify","importante","O funcionamento dos bancos é das <b>07:00</b> as <b>18:00</b>.")
			end
		end
	end
end)
...
RegisterCommand("sacar",function(source,args)
	if args[1] then
		for _,v in pairs(bancos) do
			...
			if distance <= 1 then
				CalculateTimeToDisplay()
				if parseInt(hora) >= 07 and parseInt(hora) <= 18 then
