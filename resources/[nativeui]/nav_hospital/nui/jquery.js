$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("http://nav_hospital/shopClose");
		}
	};
});
...
const remediosList = () => {
	$.post("http://nav_hospital/remediosList",JSON.stringify({}),(data) => {
		let i = 0;
...
const curativosList = () => {
	$.post("http://nav_hospital/curativosList",JSON.stringify({}),(data) => {
		let i = 0;
		const nameList = data.shopitens.sort((a,b) => (a.name > b.name) ? 1: -1);
		$('#inicio').html(`
			<input id="amount" class="qtd" maxlength="7" spellcheck="false" value="" placeholder="QUANT..">
...
$(document).on("click",".comprar",function(){
	...
	if($el && amount > 0){
		$.post("http://nav_hospital/shopBuy",JSON.stringify({
