...
RegisterCommand('tweet', function(source, args, rawCommand)
	...
	if user_id ~= nil then
		...
		TriggerClientEvent('chatMessage', -1, "[@Tweet] ".. identity.name .. " " .. identity.firstname .." ("..user_id..") ", {0, 170, 255}, rawCommand:sub(6))
...
RegisterCommand('twt', function(source, args, rawCommand)
	...
	if user_id ~= nil then
		local player = vRP.getUserSource(user_id)
		local identity = vRP.getUserIdentity(user_id)
		TriggerClientEvent('chatMessage', -1, "[@Tweet] ".. identity.name .. " " .. identity.firstname .." ("..user_id..") ", {0, 170, 255}, rawCommand:sub(5))
		print("[Tweet] " ..GetPlayerName(player).. " - " ..user_id .. ':' .. rawCommand:sub(6))
	end
end)
...
RegisterCommand('twitter', function(source, args, rawCommand)
	...
	if user_id ~= nil then
		local player = vRP.getUserSource(user_id)
		local identity = vRP.getUserIdentity(user_id)
		TriggerClientEvent('chatMessage', -1, "[@Tweet] ".. identity.name .. " " .. identity.firstname .." ("..user_id..") ", {0, 170, 255}, rawCommand:sub(8))
		print("[Tweet] " ..GetPlayerName(player).. " - " ..user_id .. ':' .. rawCommand:sub(6))
	end
end)
...
RegisterCommand('olx', function(source, args, rawCommand)
	...
	if user_id ~= nil then
		local player = vRP.getUserSource(user_id)
		local identity = vRP.getUserIdentity(user_id)
		TriggerClientEvent('chatMessage', -1, "[@OLX] ".. identity.name .. " " .. identity.firstname .." ("..user_id..") ", {255, 225, 0}, rawCommand:sub(4))
...
RegisterCommand('190', function(source, args, rawCommand)
	...
	if user_id ~= nil then
		local player = vRP.getUserSource(user_id)
		local identity = vRP.getUserIdentity(user_id)
		TriggerClientEvent('chatMessage', -1, "[@190] ".. identity.name .. " " .. identity.firstname .." ("..user_id..") ", {0, 0, 255}, rawCommand:sub(4))
...
RegisterCommand('192', function(source, args, rawCommand)
	...
	if user_id ~= nil then
		local player = vRP.getUserSource(user_id)
		local identity = vRP.getUserIdentity(user_id)
		TriggerClientEvent('chatMessage', -1, "[@192] ".. identity.name .. " " .. identity.firstname .." ("..user_id..") ", {255, 120, 120}, rawCommand:sub(4))
...
RegisterCommand('id', function(source, args, rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		TriggerClientEvent('chatMessage', source, '', {255, 255, 255}, "^3Seu ID é: ^7"..user_id)
	end
end)
