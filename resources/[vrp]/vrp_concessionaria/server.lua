...
function func.comprarVeiculo(categoria, modelo)
    ...
    if func.abertoTodos() or (Config.AbertoAll == false and isVendedor) then
        ...
        if veiculo then
            ...
            if #getVeiculo == 0 then
                ...
                if parseInt(rows[1].qtd) >= veiculo.estoque then
                    ...
                    if isVendedor then valor = valor * 0.7 end
                    ...
                    if vRP.tryFullPayment(user_id, valor) then
                        ...
                        TriggerClientEvent("vrp_concessionaria:notify", source,
                                           "Oba!", "Pagou <b>$" .. vRP.format(parseInt(valor)) .. " reais</b>.", "success")
                        return true
                    else
        ...
        TriggerClientEvent("vrp_concessionaria:notify", source, "Ops!","Somente o dono da concessionária pode executar está ação!",
                           "error")
        return
    end
...
function func.venderVeiculo(categoria, modelo)
    ...
    if veiculo then
        ...
        if parseInt(price) > 0 then
            TriggerClientEvent("vrp_concessionaria:notify", source, "Oba!", "Recebeu <b>$" .. vRP.format(parseInt(price)) .. " reais</b>.", "success")
        end
        ...
        return false
         ...
         TriggerClientEvent("Notify",source,"negado", "Somente o dono da concessionária pode executar esta ação!")
     end
end
