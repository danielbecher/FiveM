...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0,256) and IsPedInAnyPoliceVehicle(PlayerPedId()) then
			if radar.shown then
				radar.shown = false
			else
				radar.shown = true
		...
		if radar.shown then
			if radar.freeze == false then
				...
				if IsEntityAVehicle(e) then
					...
					local fvspeed = GetEntitySpeed(e)*3.605936
				...
				if IsEntityAVehicle(j) then
					...
					local bvspeed = GetEntitySpeed(j)*3.605936
			...
			drawTxt(radar.info,4,0.5,0.875,0.50,255,255,255,200)
			drawTxt(radar.info2,4,0.5,0.90,0.50,255,255,255,200)
		end
