...
Citizen.CreateThread(function()
	while true do
		...
		if not IsPedInAnyVehicle(PlayerPedId()) then
			if IsControlJustPressed(0,47) then
				TriggerServerEvent("vrp_policia:algemar2")
			end
		end
	end
end)
...
AddEventHandler('notificacao',function(x,y,z,user_id)
	...
	--if distance <= 2100 then
		if not DoesBlipExist(blips[user_id]) then
			...
			TriggerEvent('chatMessage',"190",{64,64,255},"Disparos de arma de fogo aconteceram, verifique o ocorrido.")
		...
		if prisioneiro then
			...
			if distance2 <= 100 and reducaopenal then
				...
				if distance2 <= 1.2 then
					...
					if IsControlJustPressed(0,38) then
						reducaopenal = false
						TriggerServerEvent("diminuirpena5054")
						vRP._DeletarObjeto()
					end
				end
			end
