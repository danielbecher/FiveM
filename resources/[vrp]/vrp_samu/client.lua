...
emP = Tunnel.getInterface("vrp_samu")
    ...
    if vRP.isInComa() then 
    	if emP.checkServices() then
    	    if emP.checkPayment() then
    	        TriggerEvent("Notify","sucesso","Você pagou $1.500")
...
function GoToTarget(x, y, z, vehicle, driver, vehhash, target)
    ...
    while enroute do
        ...
        if distanceToTarget < 20 then
            ShowAdvancedNotification("CHAR_DR_FRIEDLANDER", "~b~SAMU", "~r~Doutor Dr. Áuzio", "Já estou chegando.")
...
function GoToTargetWalking(target, vehicle, driver)
    while enroute do
        ...
        if distanceToTarget <= 2 then
            ...
            ShowAdvancedNotification("CHAR_DR_FRIEDLANDER", "~b~SAMU", "~r~Doutor Dr. Áuzio", "O que aconteceu? Vou tentar reanimar você.")
            ...
            ShowAdvancedNotification("CHAR_DR_FRIEDLANDER", "~b~SAMU", "~r~Doutor Dr. Áuzio", "Tudo certo! Já vou indo.")
        end
